...
| `auto-merge.yml` | Merge a trusted bot PR once every check passes; trigger via `workflow_run` of `Auto Approve` and grant `contents: write`, `pull-requests: write`, `actions: read`, `checks: read`, `statuses: read` | `head-branch` |
